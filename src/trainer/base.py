...
class BaseTrainer():
    ...
    def _run_step(self):
        ...
        if self.cfg['gpu'] != 'None':
            for dataset_key in data:
                data[dataset_key]['real_noisy_template'] = data[dataset_key]['real_noisy_template'].cuda()
                data[dataset_key]['real_noisy_search'] = data[dataset_key]['real_noisy_search'].cuda()
    ...
    def test_img(self, image_dir, save_dir='./'):
        ...
        # forward
        denoised = self.denoiser(noisy)[0]
        ...
        cv2.imwrite(os.path.join(save_dir, name+'_DN.jpg'), denoised)
    ...
    @torch.no_grad()
    def test_track(self, image):
        '''
        Inference a tracking image.
        '''
        # load image
        noisy = image
